...
export class CartService {
  ...
  getListProductInCart(currentUserId) {
    ...
    return this.httpClient.get('https://localhost:3000/api/queries/listProductsInMyCart',
  ...
  buyProduct(productId, oldOwnerId, currentUserId, nameOldOwner, productName, nameNewOwner, productImage) {
    ...
    return this.httpClient.post('https://localhost:3001/api/Trade',
      transactionDetails, { withCredentials: true }).toPromise();
  }

  getProductNameById(productId) {
    return this.httpClient.get('https://localhost:3000/api/Product/' + productId, { withCredentials: true }).toPromise()
      .then((product) => {
